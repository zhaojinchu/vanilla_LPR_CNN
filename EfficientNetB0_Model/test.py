...
class LicensePlateDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return image_tensor, bbox, img_path
...
class LicensePlateDetector(nn.Module):
    ...
    def forward(self, x):
        ...
        # Returns bounding box predictions in absolute coordinates
        return bbox_preds
...
for batch_idx, (image_tensor, target_bbox, img_path) in enumerate(val_loader):
    ...
    with torch.no_grad():
        output_bbox = model(image_tensor).cpu().squeeze(0)
    print(output_bbox)
